...
/*
	...
	- 특징
		...
		4) 최소 힙 또는 최대 힙
			- 최소 힙 : 부모 노드의 값이 자식노드의 값보다 작은 경우
			- 최대 힙 : 부모 노드의 값이 자식노드의 값보다 큰 경우
	- 장점
		1) 최소값 또는 최대값을 상수 시간 (O(1))에 찾을 수 있음
		2) 정렬, 우선순위 큐 등에서 유용하게 사용
		3) 이진 탐색 트리보다 간단하고 빠르게 구현
	- 단점
		1) 중복된 값 처기가 어려움
		2) 삽입 삭제 연산에 시간 복잡도가 O(log n)이 소요
	...
	- 장점
		1) 배열로 구현할 때 간단하게 구현할 수 있다
			- i번째 노드의 왼쪽 자식 노드 = 2i 번째 노드
			- i번째 노드의 오른쪽 자식 노드 = 2i + 1 번째 노드
			- i번째 노드의 부모 노드 = i/2 번째 노드
		2) 담색 시간이 O(log n)으로 매우 빠름
		3) 노드의 개수가 적어 공간 복잡도가 적음
	- 단점
		1) 삽입과 삭제 시 노드의 이동이 많아서 시간 복잡도가 높음
...
/*
	...
	- 특징
		...
		2) 간선은 방향성과 가중치를 가질수 있음
		3) 방향 그래프, 무방향 그래프 존재
		4) 그래프 내의 경로를 탐색할 수 있음
	- 장점
		1) 현실 세계를 모델링하기에 유용
		2) 많은 문제를 그래프 모델로 변환하여 해결 가능
		3) 최단 경로, 최소 신장 트리 등 다양한 알고리즘이 존재
	- 단점
		1) 메모리를 많이 차지하기 때문에 큰 그래프를 다루기 어려울 수 있음
