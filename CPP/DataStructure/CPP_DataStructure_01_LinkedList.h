...
- 특징
	...
	3) 크기가 가변적이므로 동적 메모리 할당과 함께 사용되기도 함
- 장점
	1) 삽입, 삭제가 용이하며, 데이터의 크기가 가변적일 경우 유용
	2) 데이터를 읽는 것보다 데이터를 추가, 삭제하는데 빠름
- 단점
	1) 데이터를 찾는데 걸리는 시간이 선형적으로 증가하기 때문에 탐색속도가 느림
	2) 중간에 있는 노드를 삭제하려면 메모리 낭비가 발생할 수 있음
*/
