...
void States::CreateRasterizerDesc()
{
	...
	rsDesc.FrontCounterClockwise = false;	// 전면의 방향을 시계방향으로 설정
	...
	rsDesc.AntialiasedLineEnable = false;	// 선 안티엘리어싱 사용 설정
}
...
void States::CreateDepthStencilDesc()
{
	...
	dsDesc.BackFace.StencilDepthFailOp = D3D11_STENCIL_OP_INCR;		// 후면 면의 스텐실 깊이 실패 작업 설정
...
void States::CreateSamplerDesc()
{
	...
	spDesc.AddressU = D3D11_TEXTURE_ADDRESS_WRAP;		// U 좌표에 대해 텍스처 좌표를 사용
	spDesc.AddressV = D3D11_TEXTURE_ADDRESS_WRAP;		// V 좌표에 대해 텍스처 좌표를 사용
	spDesc.AddressW = D3D11_TEXTURE_ADDRESS_WRAP;		// W 좌표에 대해 텍스처 좌표를 사용
	spDesc.MaxAnisotropy = 1;							// 아니소트로피 필터링에 사용 (현재는 사용 안함)
	spDesc.ComparisonFunc = D3D11_COMPARISON_ALWAYS;	// 비교 샘플리에서 사용 (현재는 사용 안함)
...
/*
	...
	Stencil
		...
		각 픽셀에 대해 스텐실 값을 저장하여, 이 값은 렌더링 중에 테스트 및 업데이트
		다양한 렌더링 기술에서 사용
			ex) 그림자 볼륨, 클리핑 평면 등등
